...
## on UX, but rather the implementation... so I omitted it. Please keep
## floats under 10 digits to avoid floating point errors with specific
## geometric calculations.

## Note: this algorithm is set up such that if given a set of points that are
## all collinear, it will just return all points. Therefore it will let the line
## itself be the convex hull.
...
print = print  ##This is necessary for mocking the print function in unit test.
input = input  ##This is necessary for mocking the input function in unit test.
X_VAL = 0
Y_VAL = 1
...
def getPointsOfConvexHull(points_in_plane):
    """Takes in an array of tuples that represent a set of points in the XY plane, and returns an Array containing all points that make up
       the convex hull. Also prepares necessary lines for plotting if MatPlotLib installed on machine."""
...
def prepareLineForPlot(point1, point2):
    """Takes in two points on the XY plane and uses the external library MatPlotLib to graph a green line between the two points.
        Graphic won't appear until show() is called."""
...
def lineIsBorderOfHull(point1, point2, points_in_plane):
    ...
    for i in range(0, n):
        ...
        if getPointsPositionRelativeToLine(point, line_equation) == 0:
            ...
            if first_points_position_relative_to_line is None:
...
def constructLineBetweenPoints(point1, point2):
    """Takes in 2 points (tuples) and returns a two variable lambda representing the line between those two points. Recognizes vertical
...
def getPointsPositionRelativeToLine(point, line_equation):
    """Takes in a 2 variable lambda representing the equation of a line and any point on the plane (Tuple), then determines that points
       location in relation to the given line. To avoid floating point error we round each result to 9 digits. See preface
       instructions. Returns 0 for a collinear point."""
...
def printPointsOfConvexHull(points_of_convex_hull):
    """Displays results by printing each point (tuple) making up the convex hull."""
...
def main():
    ...
    points_in_plane = [(-2, 5), (1, -5), (2, 15), (30, 30), (10, 10), (15, 15), (30, -10), (20, 3), (25, 10), (29, 2),
                       (-10, 10), (19, 19), (10, 9.8), (50, 45), (20, 67)]
